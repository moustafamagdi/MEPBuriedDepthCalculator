...
namespace MEPBuriedDepthCalculator.Services
{
    ...
    public class DepthCalculationService
    {
        ...
        public List<ElementCalculationResult> CalculateAndApply(Document doc, List<Element> elements, CalculationOptions options, out CalculationSummary summary)
        {
            ...
            {
                _logger.Error("CalculationEngine", "No valid Revit Link with Toposolids selected. Ensure the link is loaded and contains the Finished Ground Toposolid.");
                stopwatch.Stop();
                summary = new CalculationSummary { TotalSelected = elements.Count, Duration = stopwatch.Elapsed };
                return results;
            }

            // Build the Toposolid triangulation cache ONCE per run instead of re-collecting and
            // re-triangulating for every single element endpoint (major perf fix for large models).
            var geometryCache = _toposolidService.BuildCache(selectedLink.LinkedDocument);
            if (geometryCache.ToposolidCount == 0)
            {
                _logger.Error("CalculationEngine", $"Selected link '{selectedLink.Name}' contains no Toposolid elements. No ground surfaces to evaluate against.");
                stopwatch.Stop();
                summary = new CalculationSummary { TotalSelected = elements.Count, Duration = stopwatch.Elapsed };
                return results;
            }
            ...
            foreach (var elem in elements)
            {
                ...
                try
                {
                    ...
                    var startGround = _toposolidService.FindNearestUpperGround(geometryCache, selectedLink.Transform, startPoint, startDim.BottomElevation);
                    ...
                    var endGround = _toposolidService.FindNearestUpperGround(geometryCache, selectedLink.Transform, endPoint, endDim.BottomElevation);
